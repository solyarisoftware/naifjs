...
It must be replaced asap, storing/retrieving data in/from sessions db.
...
> ⚠️  WARNING
> when NaifJs run as a server in a multi user channel, 
> in case of delayed/promised/timeout functions 
> (anything that will run out-of-time in a future (~promise), 
> please remember to pass the *session id* to functions that will be activated with a time delay.
...
NaifJs could supply a ready-made default response timeout handler, 
by example, asking every M seconds the last prompt P. 

### 5. Prompts timeout data management 
 
So far, timeout handlers are managed in a dedicated in-memory object. 
It's a debatable decision.
Maybe would be part of the session object.
...
It could be useful to generate state machine code, 
starting from a graphical representation of a state machine. 
...
To maintain the same unique code base, avoiding any duplication,
...
could be in a configuration file.
